...
#    for d in `cat $SCHED | grep -i $p| tr '[]' ' ' | head -1 | tr -s ' '|sed 's/--(Km)--/-- Km --/g'|awk -F' ' '{print $29}' | tr '()/' ' ' | awk '{print $3,$6}'| tr ' ' '\n'| uniq | awk ' BEGIN { yy=ENVIRON["yy"]};  { printf("%s-%s\n",$1,yy) }'`; do
    for d in `cat $SCHED | grep -i $p| sed 's/.*(\([[:digit:]][[:digit:]]\)\/\([[:digit:]][[:digit:]]\)).*/\1 \2/g'| uniq| head -1 | awk ' function get_mon(m) { if (m==1) {return "jan"}; if (m==2) {return "feb"}; if (m==3) {return "mar"}; if (m==4) {return "apr"}; if (m==5) {return "may"}; if (m==6) {return "jun"}; if (m==7) {return "jul"}; if (m==8) {return "aug"};  if (m==9) {return "sep"}; if (m==10) {return "oct"}; if (m == 11) {return "nov"}; if (m==12) {return "dec"}; return "nonexistant"; } BEGIN { yy=ENVIRON["yy"]};  { printf("%s%s\n",get_mon(int($2)),yy) }'`; do
	mkdir to_ftp/$d
#	echo mkdir to_ftp/$d
